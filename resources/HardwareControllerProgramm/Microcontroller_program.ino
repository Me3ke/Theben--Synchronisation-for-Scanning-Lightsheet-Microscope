...
/*
  Receives a single byte from the serial port.
...
/*
  Receives bytes of given length from the serial port.
...
/*
  Sets the DAC to given Position. Can be used to determine highest and lowest positions
*/
void setPosition() {
...
void scan() {
	...
	tFirstTrigger = tTrig + lineTime - tInitial;
...
bool timeCalibration() {
  ...
  if (number == 6) { // 6 is trigger command
	...
	long int tAll = (picHeight + expLines) * lineTime - lineTime;	// overall time needed
...
bool timeCalibration() {
	...
	int step = 0;	// number of runs
	while (true) {
		// scanning routine without triggering cam:
		slowTransition(nullPos, highPos, 1, 1);
		slowTransition(highPos, interPos, -1, 1);
		delay(1000);
		t1 = micros();
